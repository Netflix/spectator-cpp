...
Registry::Registry(const Config& config) : m_config(config) 
{
    if (config.GetWriterType() == WriterType::Memory)
    {
        ...
        Writer::Initialize(config.GetWriterType(), "", 0, this->m_config.GetWriterBufferSize(), config.GetWriterModeType());
    }
    else if (config.GetWriterType() == WriterType::UDP)
    {
        ...
        Writer::Initialize(config.GetWriterType(), ip, port, this->m_config.GetWriterBufferSize(), config.GetWriterModeType());
    }
    else if (config.GetWriterType() == WriterType::Unix)
    {
        ...
        Writer::Initialize(config.GetWriterType(), socketPath, 0, this->m_config.GetWriterBufferSize(), config.GetWriterModeType());
    }    
}
